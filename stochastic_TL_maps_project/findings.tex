...
(flat shallow, flat deep, flat shallow, upslope, downslope) and five input
uncertainty distributions (Uniform\,1/5/10\%, Normal\,5/10\%).
...
\begin{table}[H]
  ...
  \caption{Median KS statistic for LN3 fit to MC TL samples.
           ...
           $\dagger$~Values require reading cached LN3 results; run \texttt{fill\_findings.m}
           to populate.}
  \label{tab:ks_ln3}
  \begin{tabular}{lcc}
    \toprule
    ...
    \midrule
    Baseline      & $\dagger$ & $\dagger$ \\
    Deep water    & $\dagger$ & $\dagger$ \\
    Shallow water & $\dagger$ & $\dagger$ \\
    Upslope       & $\dagger$ & $\dagger$ \\
    Downslope     & $\dagger$ & $\dagger$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \caption{Relative $L_1$ error of 1\textsuperscript{st}-order Delta variance vs
           ...
           $\dagger$~Run \texttt{fill\_findings.m} to populate from cached results.}
  \label{tab:analytical_l1}
  \begin{tabular}{lccc}
    \toprule
    Distribution & $\sigma_{z_S}$ (m) & $L_1$ (Analytical $J$) & $L_1$ (FD $J$) \\
    \midrule
    Normal\,1\%  & 0.5  & $\dagger$ & $\dagger$ \\
    Normal\,5\%  & 2.5  & $\dagger$ & $\dagger$ \\
    Normal\,10\% & 5.0  & $\dagger$ & $\dagger$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
Our results show $K^* = 1$ for frequency in the baseline scenario (consistent with
...
$K^*$ by approximately 2--3 orders in deep water (e.g.\ Uniform\,1\%: $K^* = 3$;
Uniform\,10\%: $K^* = 4$--$5$).
...
The Normal distribution tends to require 1--2 higher orders than the Uniform
