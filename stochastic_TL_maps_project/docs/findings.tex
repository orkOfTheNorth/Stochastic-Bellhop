...
\graphicspath{
  ...
  {../Methods/Methods/MC_convergence/}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \caption{MC vs.\ Bellhop-FD Delta}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.48\textwidth}
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \caption{MC vs.\ LN3}
  \end{subfigure}
  \caption{Bellhop Finite-Difference Jacobian (Bellhop-FD) Delta vs.\ MC vs.\ Delta--LN3
           shadow-probability comparison ($P(\TL > 70\,\text{dB})$) where the analytical
           methods \emph{agree} with MC (baseline flat scenario, Uniform\,1\%).
           In each panel MC appears on the left and the analytical approximation on the
           right; close match indicates the method is reliable in this low-perturbation
           flat-bathymetry regime.}
...
\begin{figure}[H]
  ...
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.48\textwidth}
    \includegraphics[width=\linewidth]{baseline/Normal_10pct/figures/shadow_70_MC_vs_DeltaLN3.png}
    \caption{MC vs.\ Delta--LN3}
  \end{subfigure}\\[8pt]
  \begin{subfigure}[t]{0.48\textwidth}
    \includegraphics[width=\linewidth]{baseline/Normal_10pct/figures/shadow_70_MC_vs_LN3.png}
    \caption{MC vs.\ LN3}
  \end{subfigure}
  \caption{Bellhop Finite-Difference Jacobian (Bellhop-FD) Delta vs.\ MC vs.\ Delta--LN3
           shadow-probability comparison ($P(\TL > 70\,\text{dB})$) where the
           1\textsuperscript{st}-order Delta \emph{begins to fail} (baseline scenario,
           Normal\,10\% --- large perturbation in flat geometry).
           In each panel MC is on the left and the analytical approximation on the right.
           The disagreement grows at shadow boundaries where the Hessian correction
           already exceeds 99\% of the total variance (Table~\ref{tab:2nd_order_correction}).
           Slope scenarios (downslope, upslope) show even larger failure but their
           Comparison figures require re-running the pipeline with Normal distributions.}
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.48\textwidth}
    ...
    \caption{Baseline (flat, 50\,m) --- 2\textsuperscript{nd}/1\textsuperscript{st} order ratio.}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.48\textwidth}
    \includegraphics[width=\linewidth]{downslope/delta_order_diff_relative.png}
    \caption{Downslope --- 2\textsuperscript{nd}/1\textsuperscript{st} order ratio.}
  \end{subfigure}
  \caption{Relative magnitude of the 2\textsuperscript{nd}-order Hessian correction
           compared to the 1\textsuperscript{st}-order (Bellhop-FD Jacobian) variance
           estimate, across all range--depth pixels.
           Red pixels: 2\textsuperscript{nd}-order correction dominates ($> 50\%$) ---
           the 1\textsuperscript{st}-order Taylor approximation has not converged.
           Left: baseline flat geometry --- correction is concentrated at interference
           nulls but is moderate in the propagation corridor.
           Right: downslope --- correction exceeds 80\% across almost the entire domain,
           confirming that no 2\textsuperscript{nd}-order correction suffices here.}
  \label{fig:delta_order_ratio}
\end{figure}
...
\begin{figure}[H]
  \centering
  \begin{subfigure}[t]{0.48\textwidth}
    \includegraphics[width=\linewidth]{baseline/Normal_5pct/figures/var_MC_vs_Delta_3x2.png}
    \caption{Baseline (flat, 50\,m) --- Normal\,5\%.}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.48\textwidth}
    \includegraphics[width=\linewidth]{baseline/Normal_10pct/figures/var_MC_vs_Delta_3x2.png}
    \caption{Baseline (flat, 50\,m) --- Normal\,10\%.}
  \end{subfigure}
  \caption{MC variance $\Var_\text{MC}[\TL]$ vs.\ Bellhop Finite-Difference Jacobian
           (Bellhop-FD) Delta variance $\Var_\Delta[\TL]$ for each of the three uncertain
           parameters (rows: $z_S$, freq, SVP) and the full three-parameter combination.
           Left: 5\% perturbation --- Delta is a reasonable approximation for freq and SVP
           but overestimates variance for $z_S$ in the convergence zone.
           Right: 10\% perturbation --- systematic over/under-estimation across all parameters
           confirms that the 1\textsuperscript{st}-order Taylor series has not converged.}
...
Figure~\ref{fig:pce_convergence_a} shows the
relative $L_1$ error $= 1 - R^2_K$ as a function of PCE order $K$
...
\begin{figure}[H]
  \centering
  \input{../validation/analytical/analytical_diagram}
...
\textit{Fundamentals of Ocean Acoustics}, 2nd ed.\
