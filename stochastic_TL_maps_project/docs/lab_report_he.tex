...
המטרה היא להסביר מדוע מפת \textenglish{TL} דטרמיניסטית בודדת של \textenglish{Bellhop}
אינה מספיקה לתכנון פעולת סונאר, ולספק את הרקע התיאורטי לשיטות ה-\textenglish{UQ}
הנבחנות ביתר הדוח.
...
\begin{figure}[H]
  ...
  \caption{הים כמדיום אקוסטי שכבתי: מבנה פרופיל SVP ואפקט ה-\textenglish{SOFAR}.
           \src{run\_mc.m: cfg = loadConfig(); plot(svp\_c, svp\_z)}}
...
השדה האקוסטי בים מתואר על ידי משוואת הלמהולץ בגיאומטריה סילינדרית סימטרית ($r$-$z$):
\begin{equation}
...
\end{equation}
\textenglish{Bellhop}~\cite{porter1987bellhop} פותר משוואה זו בקירוב הקרן הגאוסיאנית:
אנרגיה אקוסטית מתפשטת לאורך קרנות, כאשר כל קרן נושאת מעטפת גאוסיאנית
המטפלת בעקיפה ללא הסינגולאריות של אופטיקה גיאומטרית פשוטה.
...
בסביבה סטוכסטית, זה הופך להסתברות:
\begin{equation}
  \Pshadow(\mathbf{x}) = P\!\bigl(\TL(\mathbf{x};\boldsymbol{\theta}) > \mathrm{FOM}\bigr).
\end{equation}
הסתברות הצל $\Pshadow$ היא פלט ה-\textenglish{UQ} העיקרי שמעניין אותנו.
...
\begin{itemize}
  \item \textbf{אזורי התכנסות (\textenglish{CZ})}: מנגנון המיקוד של ערוץ הקול
        מייצר קשתות מחזוריות של TL נמוך (אנרגיה גבוהה) בכפולות של טווח ה-\textenglish{CZ}
        ($\sim$55~ק"מ עבור SVP טיפוסי במים עמוקים).
        מיקום ה-\textenglish{CZ} רגיש מאוד ל-SVP קרוב לפני המים: הפרעת SVP של 5\%{}
        יכולה להזיז את קשת ה-\textenglish{CZ} ב-$\pm$5--10~ק"מ ולשנות את רווח ה-\textenglish{CZ}
        ב-$\pm$5--10~dB.
  \item \textbf{החזרות מקרקעית הים}: במים רדודים ובסביבות מדרון,
        אנרגיה אקוסטית קופצת בין המשטח לקרקעית, ויוצרת תבניות הפרעה של מראת לויד
