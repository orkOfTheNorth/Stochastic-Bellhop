...
for ci = 1:numel(completed)
    ...
    for pi = 1:3
        ...
        % C_all will be filled below with the K*-order QR coefficients (see below).
        % The previous ridge approach used lambda = 1e-10 * trace(Phi'*Phi) which
        % is dominated by He_20 (norm^2 ~ N*20! ~ 2.4e21), making lambda ~ 2.4e11
        % and forcing all coefficients to zero.  We instead save c_best at K*.
        ...
        % Save K*-order QR coefficients as C_all (padded with zeros for unused orders)
        c_padded          = zeros(MAX_ORDER+1, Npix);
        c_padded(1:Ks+1,:) = c_best;
        C_all{pi}         = c_padded;
    ...
    %% ── Figure 3: Distribution comparison ───────────────────────────────────
        ...
        if ~isfile(dist_fig_path)
            ...
            for pi = 1:3
                ...
                % Use K* coefficients only — using K=MAX_ORDER causes Hermite blow-up
                % at |xi|>2 (He_20(3)~3.5e9), producing nonsensical PCE predictions.
                Kp = max(1, Kstar(pi));
                Phi_new = pce_basis(xi_new, Kp, dist_type);
                y_pce   = Phi_new * C_all{pi}(1:Kp+1, :);
                ...
                for ci_px = 1:3
                    ...
                    tl_pce = y_pce(:, pix);
                    all_v  = [tl_mc; tl_pce];
                    edges  = linspace(prctile(all_v,1), prctile(all_v,99), 52);
