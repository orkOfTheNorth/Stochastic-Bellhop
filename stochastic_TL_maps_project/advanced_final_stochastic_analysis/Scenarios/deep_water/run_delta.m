...
% Bathymetry: flat 2500 m bottom, range 50 km
% Parameters: freq, z_S, svp_noise  (±1%)
...
fig1 = figure('Name',sprintf('Delta %s – Single',SCN_LABEL),'Position',[30 30 1500 720]);
for k=1:3
    subplot(2,3,k);
    ...
    title(sprintf('E[TL] | %s',subset_labels{k}));
    subplot(2,3,3+k);
    pcolor(r_km,z_m,Var_delta{k}); shading interp; set(gca,'YDir','reverse');
    colormap(gca,hot); colorbar;
    xlabel('Range (km)'); ylabel('Depth (m)');
    title(sprintf('Var[TL] | %s',subset_labels{k}));
end
sgtitle(sprintf('Delta – Single-Param | %s | FOM=%ddB',SCN_LABEL,FOM));
