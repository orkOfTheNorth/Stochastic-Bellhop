...
% Bathymetry: flat 2500 m, range 50 km.  N=50 per subset (7×50 = 350 Bellhop runs).
% Estimate: ~4-8 s/run → 23-47 min total.
...
%% ── SCENARIO ──────────────────────────────────────────────────────────────
SCN_LABEL  = 'Deep Water (2500m flat, 50km)';
...
[~,r_grid,z_grid] = bellhopCached(sim_nom,cache_dir);
close all;
r_km=r_grid/1000;  z_m=z_grid;
[Nz,Nr]=size(bellhopCached(sim_nom,cache_dir));
...
grid on;
saveFig(fig_svp, fullfile('figures','svp_profiles')); close(fig_svp);

fprintf('=== MC | %s | N=%d ===\n',SCN_LABEL,N);
fprintf('Total Bellhop runs: %d × %d = %d\n',7,N,7*N);
...
sgtitle(sprintf('MC E[TL]  N=%d  |  %s',N,SCN_LABEL));
...
sgtitle(sprintf('MC Var[TL]  N=%d  |  %s',N,SCN_LABEL));
...
sgtitle(sprintf('MC Chebyshev P(shadow)  N=%d  |  %s',N,SCN_LABEL));
...
sgtitle(sprintf('MC Empirical P(shadow)  N=%d  |  %s',N,SCN_LABEL));
...
sgtitle(sprintf('MC LN3 P(shadow)  N=%d  |  %s',N,SCN_LABEL));
