...
switch nargin
   case 2   % straight call
      [ PlotTitle, ~, freqVec, ~, ~, Pos, pressure ] = read_shd( filename );
      freq = freqVec( 1 );
   case 3   % a frequency has been selected
      freq = varargin{ 3 };
      [ PlotTitle, ~, freqVec, ~, ~, Pos, pressure ] = read_shd( filename, freq );
   case 5   % a subplot m n p has been selected
      m = varargin{ 3 };
      n = varargin{ 4 };
      p = varargin{ 5 };      
      [ PlotTitle, ~, freqVec, ~, ~, Pos, pressure ] = read_shd( filename );
      freq = freqVec( 1 );
   case 6   % a frequency and a subplot m n p has been selected
      freq = varargin{ 3 };
      m    = varargin{ 4 };
      n    = varargin{ 5 };
      p    = varargin{ 6 };      
...
if ( size( tlt, 1 ) > 1 && size( tlt, 2 ) > 1 )
   ...
   figure(69)
   set(gcf, 'Position', [423   406   797   498]);
   h = pcolor( rt, zt, tlt );  ...
      shading flat
   colormap( tej )
   ...
   %
   hold on
   yInd = round(linspace(1, size(tlt,1), 75));
   xInd = round(linspace(1, size(tlt,2), 150));
   rtf = rt(xInd);
   ztf = zt(yInd);
   tltf = tlt(yInd,xInd);
   %contourf(rtf, ztf, -tltf, [-FOM, -FOM], 'color', 'w', 'FaceAlpha', 1)%'linewidth', 1); %1:size(tltf,2), 1:size(tltf,1),
   contour(rtf, ztf, -tltf, [-FOM, -FOM], 'color', 'w', 'linewidth', 1);
   %}
