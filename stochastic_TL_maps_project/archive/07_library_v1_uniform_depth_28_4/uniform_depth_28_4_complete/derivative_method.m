...
addpath(fullfile(fileparts(mfilename('fullpath')), 'Code', 'Functions'));
...
%% Plot + Save PNG
fig = figure('Name', 'Delta Method', 'Position', [100 100 1200 500]);
subplot(1,2,1); imagesc(r_grid/1000, z_grid, E_TL_delta);
title('E[TL]  (Delta Method)'); colorbar; set(gca,'YDir','reverse'); colormap('jet');
xlabel('Range (km)'); ylabel('Depth (m)');
subplot(1,2,2); imagesc(r_grid/1000, z_grid, Var_TL_delta);
title(sprintf('Var[TL]  z\\in[%d,%d]m  dz=%.1fm', z_min, z_max, dz));
colorbar; set(gca,'YDir','reverse'); colormap('hot');
xlabel('Range (km)'); ylabel('Depth (m)');
clim_max = prctile(Var_TL_delta(:), 95);
if clim_max > 0 && ~isnan(clim_max), clim([0 clim_max]); end
exportgraphics(fig, fullfile(fileparts(mfilename('fullpath')), 'delta_method_results.png'), 'Resolution', 300);
fprintf('Figure saved: delta_method_results.png\n');
