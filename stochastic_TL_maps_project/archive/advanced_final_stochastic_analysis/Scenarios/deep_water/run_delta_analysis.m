function run_delta_analysis(config)
% run_delta_analysis  –  Core Delta Method UQ analysis function.
%   Accepts a config struct and performs the full analysis for that scenario.
    ...
    svp_nom_prof  = makeSVPNoise(0,       max_depth);
    ...
    plot(svp_nom_prof(:,2),  svp_nom_prof(:,1),  'k-', 'LineWidth',2.5, 'DisplayName','Nominal (0 deg C)');
    ...
    saveFig(fig_svp, fullfile(figures_dir,'svp_perturbations'));
    close(fig_svp);

    set(0,'DefaultFigureVisible','on'); close all;

    %% ── JACOBIANS AND SUBSETS ────────────────────────────────────────────────
    ...
    save(fullfile(results_dir,'delta_jacobians.mat'), ...
    ...
    Var_delta = cell(1,7);  Cheb_lb = cell(1,7);
    ...
    for s = 1:7
        ...
        save(fullfile(results_dir,sprintf('delta_%s.mat',subset_names{s})), ...
             'TL_expected','Var_TL','Cheb_lb_s','Cheb_shadow95','r_grid','z_grid','r_km','z_m','FOM');
        fprintf('  Saved delta_%s\n',subset_names{s});
    end
    ...
    saveFig(fig4,fullfile(figures_dir,'delta_all_subsets_mean_TL'));  close(fig4);
    ...
    saveFig(fig5,fullfile(figures_dir,'delta_all_subsets_variance'));  close(fig5);
    ...
    sgtitle(sprintf('Delta Chebyshev P(shadow)  FOM=%ddB | %s',FOM,SCN_LABEL));
    saveFig(fig6,fullfile(figures_dir,'delta_all_subsets_cheb'));  close(fig6);
